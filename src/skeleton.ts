...
export class Skeleton {
  ...
  private draw3d(pose: Pose){
    const leftEye = pose.keypoints.find(
      (keypoint) => keypoint.name === "left_eye"
    );
    const rightEye = pose.keypoints.find(
      (keypoint) => keypoint.name == "right_eye"
    );
    const leftMouth = pose.keypoints.find(
      (keypoint) => keypoint.name === "mouth_left"
    );
    const rightMouth = pose.keypoints.find(
      (keypoint) => keypoint.name === "mouth_right"
    );
  ...
  private drawLeftHand(pose: Pose) {
    ...
    const leftIndex = pose.keypoints.find(
      (keypoint) => keypoint.name === "left_index"
    );
    const leftPinky = pose.keypoints.find(
      (keypoint) => keypoint.name === "left_pinky"
    );
  ...
  private drawRightHand(pose: Pose) {
    ...
    const rightIndex = pose.keypoints.find(
      (keypoint) => keypoint.name === "right_index"
    );
    const rightPinky = pose.keypoints.find(
      (keypoint) => keypoint.name === "right_pinky"
    );
